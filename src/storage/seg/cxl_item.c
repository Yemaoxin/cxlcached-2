...
_item_link(struct cxl_item *it, bool relink)
{
    ...
    cxl_hashtable_put(it, hash_table);
...
void
item_relink(struct cxl_item *it)
{
    _item_link(it, true);
}
 ...
 */
struct cxl_item *
cxl_item_get(const struct bstring *key)
{
    struct cxl_item *it;

    it = cxl_hashtable_get(key->data, key->len, hash_table);
    if (it == NULL) {
...
void
item_backfill(struct cxl_item *it, const struct bstring *val)
{
    ASSERT(it != NULL);
