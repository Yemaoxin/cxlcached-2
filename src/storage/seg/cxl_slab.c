...
_slab_to_item(struct slab *slab, uint32_t idx, size_t size)
{
    ...
    it = (struct cxl_item *)((char *)slab->data + offset);
 ...
 */
static void
_slab_put_item_into_freeq(struct cxl_item *it, uint8_t id)
{
...
_slab_recreate_items(struct slab *slab)
{
    ...
    for (i = 0; i < p->nitem; i++) {
        ...
        if (it->is_linked) {
            p->next_item_in_slab = (struct cxl_item *)&slab->data[0];
...
//add by yemaoxin,2023-09-19 19:44:44 设置segment
void
...
//add by yemaoxin,2023-09-19 19:48:09 存在问题，即不存在容量上的限制
static struct slab *
...
_slab_evict_one(struct slab *slab)
{
    ...
    for (i = 0; i < p->nitem; i++) {
        ...
        if (it->is_linked) {
            ...
            it->in_freeq = 0;
            p->nfree_itemq--;
            //add by yemaoxin,2023-09-19 19:56:33 注意此处是使用的宏替换
            SLIST_REMOVE(&p->free_itemq, it, cxl_item, i_sle);
        }
    }
...
_slab_init(struct slab *slab, uint8_t id)
{
    ...
    for (i = 0; i < p->nitem; i++) {
        ...
        cxl_item_hdr_init(it, offset, id);
    }
 ...
 */
//add by yemaoxin,2023-09-19 18:51:24 会涉及到内存的分配问题
static struct cxl_item *
_slab_get_item(uint8_t id)
{
   ...
   //add by yemaoxin,2023-09-19 18:50:40  如果当前slab分配不到item，则分配一个新的slab
...
//add by yemaoxin,2023-09-19 18:49:23 即从slab上获取一个item chunk
struct cxl_item *
 ...
 */
void
slab_put_item(struct cxl_item *it, uint8_t id)
{
     ASSERT(!(it->in_freeq));
