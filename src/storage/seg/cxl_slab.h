...
item_to_slab(struct cxl_item *it)
{
    struct slab *slab;

    ASSERT(it->offset < slab_size);
    //add by yemaoxin,2023-09-19 18:59:37 通过offset倒推得到slab的地址
...
{
    return slab_id(cxl_item_ntotal(klen, vlen, mlen));
}

void slab_setup(slab_options_st *options, slab_metrics_st *metrics);
void slab_teardown(void);

struct cxl_item *slab_get_item(uint8_t id);
void slab_put_item(struct cxl_item *it, uint8_t id);
